...
在强化学习框架中，我们将出租车调度问题建模为一个马尔可夫决策过程 (Markov Decision Process, MDP)。MDP 由五元组 $\langle \mathcal{S}, \mathcal{A}, P, R, \gamma \rangle$ 定义，其中：

- $\mathcal{S}$：状态空间，包含订单、车辆、路网等信息
- $\mathcal{A}$：动作空间，表示调度决策
- $P(s'|s, a)$：状态转移概率
- $R(s, a, s')$：奖励函数
- $\gamma \in [0, 1]$：折扣因子

强化学习的目标是找到最优策略 $\pi^*$，使得从初始状态出发的累积奖励期望最大化：
...
核心概念是**动作值函数** (Q-function)，定义为在状态 $s$ 采取动作 $a$ 后的期望累积折扣奖励：

$$Q^\pi(s, a) = \mathbb{E}_{\tau \sim \pi} \left[\sum_{t=0}^{\infty} \gamma^t r_t \mid s_0 = s, a_0 = a\right]$$

最优动作值函数 $Q^*(s, a)$ 满足**贝尔曼最优性方程**：

$$Q^*(s, a) = \mathbb{E}_{s' \sim P(\cdot|s,a)} \left[r(s, a, s') + \gamma \max_{a'} Q^*(s', a')\right]$$

这个方程表明：当前状态-动作对的最优值等于即时奖励加上下一个状态最优值的折扣期望。

**关键参数的含义**：

- $r(s, a, s')$：从状态 $s$ 执行动作 $a$ 到达状态 $s'$ 时获得的即时奖励
- $\gamma = 0.99$：折扣因子，平衡近期和远期奖励。较大的 $\gamma$（如 0.99）表示重视长期收益，适合出租车调度这类需要长期规划的任务
- 最优策略可以通过贪心策略获得：$\pi^*(s) = \arg\max_a Q^*(s, a)$

#### 3.3.1.3 DQN 的核心思想与损失函数

DQN (Deep Q-Network) 的创新在于使用深度神经网络参数化 Q 函数。我们使用参数为 $\theta$ 的神经网络 $Q(s, a; \theta)$ 来近似最优动作值函数 $Q^*(s, a)$。

为了学习这个参数，我们构造一个**有监督学习问题**。对于每条经验 $(s, a, r, s', \text{done})$，我们定义 TD 目标 (Temporal Difference Target)：

$$y = r + \gamma \max_{a'} Q(s', a'; \theta^-) \cdot (1 - \text{done})$$

其中 $(1 - \text{done})$ 项用于处理回合结束：当 $\text{done} = 1$ 时（回合结束），TD 目标仅为 $r$。

DQN 通过最小化以下**均方误差损失函数**来学习参数 $\theta$：

$$\mathcal{L}(\theta) = \mathbb{E}_{(s,a,r,s',\text{done}) \sim \mathcal{D}} \left[\left(y - Q(s, a; \theta)\right)^2\right]$$

$$= \mathbb{E}_{(s,a,r,s',\text{done}) \sim \mathcal{D}} \left[\left(r + \gamma \max_{a'} Q(s', a'; \theta^-) \cdot (1 - \text{done}) - Q(s, a; \theta)\right)^2\right]$$

#### 3.3.1.4 目标网络的稳定性作用

标准的 Q 学习使用同一个网络计算当前 Q 值和目标 Q 值，这会导致**自举问题** (bootstrapping problem)：目标依赖于要优化的参数，导致训练不稳定。

DQN 的关键创新是引入**目标网络** $Q(\cdot; \theta^-)$，其参数 $\theta^-$ 与主网络 $\theta$ 分离。目标网络用于计算 TD 目标中的 $\max_{a'} Q(s', a'; \theta^-)$，而主网络用于计算当前 Q 值 $Q(s, a; \theta)$ 和更新梯度。

这种分离的好处：

1. **稳定性**：目标 $y$ 在多个训练步骤内保持相对稳定，减少了目标移动的问题
2. **收敛性**：有利于梯度下降的收敛
3. **方差减少**：减少由于参数变化导致的目标方差

目标网络定期从主网络复制参数：

$$\theta^- \leftarrow \theta$$

在我们的实现中，更新频率为每 1000 个训练步骤更新一次。

#### 3.3.1.5 经验回放的作用

在线 Q 学习中，连续的样本具有强相关性（时间相关性），这会导致：

1. **样本效率低**：相邻样本提供的信息冗余
2. **参数更新不稳定**：相关样本会导致参数更新方向不稳定

DQN 采用**经验回放** (Experience Replay) 机制，将交互过程中产生的经验 $(s, a, r, s', \text{done})$ 存储在一个缓冲区 $\mathcal{D}$ 中。训练时，从缓冲区中**随机采样** mini-batch 进行梯度更新，这样做的优势：

1. **打破相关性**：随机采样使得训练数据近似独立同分布
2. **样本重用**：同一条经验可能被多次使用，提高样本效率
3. **稳定性**：减少参数更新的方差

经验回放缓冲区的大小为 $|\mathcal{D}| = 50,000$，采用 FIFO (First-In-First-Out) 策略，当缓冲区满时，最旧的经验被删除。

### 3.3.2 优先级经验回放 (Prioritized Experience Replay, PER)

#### 动机

标准的 DQN 从经验回放缓冲区中**均匀随机采样**。这种方法的问题：

1. **样本效率低**：所有样本被等同对待，但有些样本对学习更重要
2. **学习不稳定**：模型可能反复学习简单的样本，而忽视难样本

**优先级经验回放** 的核心思想：优先采样 **TD 误差较大** 的样本，因为这些样本对学习贡献更大。
...
- $|TD\_error_i| = |r_i + \gamma \max_{a'} Q(s_i', a'; \theta^-) - Q(s_i, a_i; \theta)|$ 是样本 $i$ 的 TD 误差
- $\alpha \in [0, 1]$ 控制优先级的强度
  - $\alpha = 0$：均匀采样（标准 DQN）
  - $\alpha = 1$：完全按 TD 误差优先级采样

**重要性采样权重**：

由于优先级采样会引入偏差（改变了数据分布），我们需要通过重要性采样权重进行补偿：

$$w_i = \left(\frac{1}{N \cdot p_i}\right)^\beta$$

其中 $\beta \in [0, 1]$ 控制补偿强度：
- $\beta = 0$：无补偿
- $\beta = 1$：完全补偿

在实际训练中，$\beta$ 从初始值逐渐增加到 1，使得模型逐步适应优先级采样的偏差。
...
| $\beta_{\text{frames}}$ | 100,000 | 在 100K 步后 $\beta$ 达到 1.0 |
| 缓冲区大小 | 50,000 | 存储最近 50K 个经验 |

这些参数的选择基于以下考虑：

1. **$\alpha = 0.4$**：相对温和的优先级强度，避免过度偏向高 TD 误差样本
2. **$\beta$ 的渐进增加**：使模型逐步适应优先级采样的偏差
3. **缓冲区大小**：在内存和样本多样性之间的折衷

### 3.3.3 目标网络与双 DQN

#### 问题：高估偏差 (Overestimation Bias)

标准 DQN 使用贪心策略选择下一个动作的 Q 值：

$$\max_{a'} Q(s', a'; \theta^-)$$

这会导致**高估偏差**：由于 Q 值的不确定性，贪心选择往往倾向于选择被高估的 Q 值。

#### 双 DQN 的改进 (Double DQN)
