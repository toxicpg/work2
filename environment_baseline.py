...
"""
Baseline 环境 - 基于主环境 environment.py (V5.3) 完整复制
专门为 Baseline 算法准备，支持外部 Agent 控制调度

核心修改（相比主环境）：
1. 类名改为 BaselineEnvironment
2. 添加 dispatch_policy 参数，支持 'none', 'random_walk', 'random_dispatch'
3. step() 方法支持外部 Agent 在调用前执行调度
4. 移除 RL 相关的初始化（model, replay_buffer 等）
"""
