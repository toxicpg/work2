...
class VehicleManager:
    ...
    def update_serving_vehicles(self, current_time):
        """更新正在服务的车辆状态（简化版，与主环境保持一致）

        注意：此方法在 baseline 环境中不再处理订单完成，
        而是通过 _schedule_order_completion 和事件队列来处理（与主环境一致）
        """
        # 简化版：baseline 环境不需要复杂的 serving 状态更新
        # 订单完成通过事件队列处理
        return [], []
    ...
    def match_orders(self, pending_orders, vehicle_manager, current_time):
        ...
        for order in unmatched_orders:
            ...
            for _, v_id, v_grid in k_nearest:
                ...
                # 预判超时：如果 (当前等待时间 + 接驾时间) > 最大等待时间，则不匹配该车辆
                # 这样可以避免产生“注定失败”的匹配，降低虚高的匹配率
                if 'timestamp' in order:
                    try:
                        ...
                        current_wait_sec = (current_time - gen_time).total_seconds()
                        pickup_time_sec = travel_time * 60.0

                        if current_wait_sec + pickup_time_sec > self.config.MAX_WAITING_TIME:
                            continue
                    except Exception:
                        pass
            ...
            # ★★★ 在匹配时立即assign，确保车辆状态同步（与主环境一致）★★★
            if best_match_vehicle_id is not None:
                # 立即分配订单（简化版：只传 vehicle_id 和 order）
...
class BaselineEnvironment:
    ...
    def step(self):
        ...
        try:
            # 1. 处理到期的订单完成事件
            self._process_order_completion_events(self.current_time, step_info)

            # 2. 更新车辆状态（dispatching车辆）
            ...
            if self.pending_orders:
                ...
                for match in matches:
                    ...
                    order['matched_time'] = self.current_time
                    ...
                    if 'timestamp' in order:
                        try:
                            ...
                            order['wait_to_match_sec'] = max(0, wait_time_sec)
                        except Exception:
                            order['wait_to_match_sec'] = 0

                    # 统计收入
                    step_info['revenue'] += order.get('fee', 0.0)

                    # 安排订单完成事件
                    self._schedule_order_completion(match, self.current_time)
    ...
    def _process_order_completion_events(self, current_time, step_info):
        """处理到期的订单完成事件"""
        due_events_indices = []
        ...
        for i, event in enumerate(self.event_queue):
            ...
            elif event_time.tzinfo != current_time.tzinfo and event_time.tzinfo is not None:
                try:
        ...
        for i in reversed(due_events_indices):
            try:
                ...
                if event.get('type') == 'order_completion':
                    vehicle_id = event.get('vehicle_id')
                    order = event.get('order')
                    dest_grid = event.get('destination_grid')

                    if vehicle_id is None or order is None or dest_grid is None:
                        continue
                    ...
                    # 完成服务
                    service_completed = self.vehicle_manager.complete_service(
                        vehicle_id, dest_grid, current_time
                    )
                    ...
                    if service_completed:
                        ...
                        if 'timestamp' in order:
                            try:
                                ...
                                # 等待时间 = 从生成到匹配的时间
                                wait_time_sec = order.get('wait_to_match_sec', 0)
                                step_info['waiting_times'].append(wait_time_sec)
                            except Exception:
                                pass
